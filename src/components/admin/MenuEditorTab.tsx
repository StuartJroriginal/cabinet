  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2.5">
        ...
        {!isBuiltin && (
          <span className="text-dark-500" title={t('admin.menuEditor.urlLabel')}>
            <LinkIcon />
          </span>
        )}
        ...
        <div className="space-y-3 border-t border-dark-700/30 px-3 py-3">
          ...
          {!isBuiltin && (
            <>
              <div>
                <label className="mb-1.5 block text-xs font-medium text-dark-300">
                  {t('admin.menuEditor.urlLabel')}
                </label>
                <input
                  type="url"
                  value={button.url || ''}
                  onChange={(e) => onUpdate({ url: e.target.value || null })}
                  placeholder={t('admin.menuEditor.urlPlaceholder')}
                  className="w-full rounded-lg border border-dark-600 bg-dark-700/50 px-3 py-2 text-sm text-dark-100 placeholder-dark-500 transition-colors focus:border-accent-500 focus:outline-none"
                />
              </div>
              <div>
    ...
    <div className="space-y-4">
      ...
      <Dialog open={isResetConfirmOpen} onOpenChange={setIsResetConfirmOpen}>
        <DialogContent showCloseButton={false} className="w-full max-w-md rounded-2xl border-dark-600 bg-dark-900 p-5">
          <DialogHeader>
            <DialogTitle>{t('admin.buttons.resetAll')}</DialogTitle>
            <DialogDescription>{t('admin.menuEditor.resetConfirm')}</DialogDescription>
          </DialogHeader>
          <DialogFooter className="mt-5 flex flex-wrap justify-end gap-2">
            <button
              type="button"
              className="rounded-xl bg-dark-700 px-4 py-2 text-sm text-dark-300 transition-colors hover:bg-dark-600"
