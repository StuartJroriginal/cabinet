...
export default function TopUpAmount() {
  ...
  const [manualCreditInfo, setManualCreditInfo] = useState<{
    required: boolean;
  ...
  >({
    ...
    onSuccess: (data) => {
      ...
      if (redirectUrl) {
        ...
        setManualCreditInfo({
          required: Boolean(data.requires_manual_credit),
          etaMinutes: data.manual_credit_eta_minutes ?? null,
          message: data.manual_credit_message ?? null,
          supportContactUrl: data.support_contact_url ?? null,
        });
        ...
        if (method && data.payment_id) {
          ...
          saveTopUpPendingInfo({
            ...
            created_at: Date.now(),
            ...(data.requires_manual_credit
              ? {
                  requires_manual_credit: true,
                  manual_credit_eta_minutes: data.manual_credit_eta_minutes ?? undefined,
                  manual_credit_message: data.manual_credit_message ?? undefined,
                  support_contact_url: data.support_contact_url ?? undefined,
                }
              : {}),
          });
        }
    ...
    onError: (err: unknown) => {
      const axiosError = err as {
        response?: { data?: { detail?: string }; headers?: Record<string, string> };
      };
      const detail = axiosError?.response?.data?.detail || '';
      const retryAfterRaw = axiosError?.response?.headers?.['retry-after'];
      const retryAfter = retryAfterRaw ? Number.parseInt(String(retryAfterRaw), 10) : NaN;
      if (!Number.isNaN(retryAfter) && retryAfter > 0) {
        setError(t('balance.errors.rateLimit', { seconds: retryAfter }));
        return;
      }
      setError(
  ...
  return (
    ...
    >
      ...
      <motion.div variants={staggerItem}>
        <BentoCard className="border border-accent-500/25 bg-accent-500/10 p-4">
          <div className="text-xs font-semibold uppercase tracking-wide text-accent-300">
            {t('balance.paymentExpectationLabel', 'Ожидаемое время')}
          </div>
          <p className="mt-2 text-sm text-dark-200">
            {t(
              'balance.paymentExpectationHint',
              'После оплаты зачисление обычно занимает 1-2 минуты. Если платёж задержался, откройте страницу результата и обратитесь в поддержку.',
            )}
          </p>
        </BentoCard>
      </motion.div>
        ...
        >
          ...
          {manualCreditInfo?.required && (
            <div className="rounded-xl border border-warning-500/30 bg-warning-500/10 p-3">
              <p className="text-sm font-semibold text-warning-300">
                {t('balance.manualCreditTitle', 'Зачисление вручную')}
              </p>
              <p className="mt-1 text-xs text-warning-200">
                {manualCreditInfo.message ||
                  t(
                    'balance.manualCreditMessage',
                    'После оплаты зачисление выполняется вручную. Обычно это занимает до {{minutes}} минут.',
                    { minutes: manualCreditInfo.etaMinutes ?? 15 },
                  )}
              </p>
              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => navigate(`/balance/top-up/result?method=${encodeURIComponent(method.id)}`)}
                  className="rounded-lg bg-warning-500 px-3 py-2 text-xs font-semibold text-dark-900 transition-colors hover:bg-warning-400"
                >
                  {t('balance.iPaid', 'Я оплатил')}
                </button>
                {manualCreditInfo.supportContactUrl && (
                  <button
                    type="button"
                    onClick={() => openLink(manualCreditInfo.supportContactUrl as string)}
                    className="rounded-lg border border-warning-400/60 px-3 py-2 text-xs font-semibold text-warning-200 transition-colors hover:bg-warning-500/20"
                  >
                    {t('common.support', 'Поддержка')}
                  </button>
                )}
              </div>
            </div>
          ...
          <div className="rounded-xl border border-dark-700/50 bg-dark-800/40 p-3">
            <p className="text-xs text-dark-300">
              {t(
                'balance.paymentDelayHint',
                'Если после оплаты баланс не обновился в течение 2-3 минут, откройте результат платежа и отправьте запрос в поддержку.',
              )}
            </p>
          </div>
