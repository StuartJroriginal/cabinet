...
export default function Balance() {
  ...
  return (
    ...
    >
      ...
      <motion.div variants={staggerItem}>
        <Card className="border border-accent-500/25 bg-accent-500/10">
          <div className="text-xs font-semibold uppercase tracking-wide text-accent-300">
            {t('balance.quickActionLabel', 'Быстрое действие')}
          </div>
          <h2 className="mt-2 text-lg font-semibold text-dark-100">
            {t('balance.quickActionTitle', 'Пополните баланс за 1 шаг')}
          </h2>
          <p className="mt-1 text-sm text-dark-300">
            {t(
              'balance.quickActionHint',
              'Выберите удобный способ оплаты и вернитесь к покупке/продлению подписки.',
            )}
          </p>
          <Button
