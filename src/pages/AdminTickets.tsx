    ...
    return (
      <div className="mt-3">
        ...
        {imageError ? (
          ...
          <img
            src={mediaUrl}
            alt={message.media_caption || t('admin.tickets.attachedImageAlt')}
  ...
  return (
    <div className="mt-3">
      ...
      >
        ...
        {message.media_caption || t('admin.tickets.downloadAttachment')}
      </a>
    </div>
  );
}

function getTicketNextStep(
  t: (key: string) => string,
  ticket: AdminTicketDetail,
): { title: string; description: string } {
  if (ticket.status === 'open') {
    return {
      title: t('admin.tickets.nextStepOpenTitle'),
      description: t('admin.tickets.nextStepOpenDesc'),
    };
  }
  if (ticket.status === 'pending') {
    return {
      title: t('admin.tickets.nextStepPendingTitle'),
      description: t('admin.tickets.nextStepPendingDesc'),
    };
  }
  if (ticket.status === 'answered') {
    return {
      title: t('admin.tickets.nextStepAnsweredTitle'),
      description: t('admin.tickets.nextStepAnsweredDesc'),
    };
  }
  return {
    title: t('admin.tickets.nextStepClosedTitle'),
    description: t('admin.tickets.nextStepClosedDesc'),
  };
}
...
export default function AdminTickets() {
  ...
  const formatUser = (ticket: AdminTicket | AdminTicketDetail) => {
    if (!ticket.user) return t('admin.tickets.unknownUser');
    const { first_name, last_name, username } = ticket.user;
    if (first_name || last_name) return `${first_name || ''} ${last_name || ''}`.trim();
    if (username) return `@${username}`;
    return t('admin.tickets.user');
  };
                ...
                >
                  ...
                  <div className="text-xs text-dark-500">
                    ...
                    {ticket.user?.telegram_id && (
                      ...
                      >
                        ({t('admin.tickets.telegramTag')}: {ticket.user!.telegram_id})
                      </button>
                    )}{' '}
        ...
        <div className="card lg:col-span-2">
          {!selectedTicketId ? (
            ...
            <div className="flex h-full flex-col">
              {(() => {
                const nextStep = getTicketNextStep(t, selectedTicket);
                return (
                  <div className="mb-4 rounded-xl border border-accent-500/20 bg-accent-500/10 p-3">
                    <p className="text-xs font-semibold uppercase tracking-wide text-accent-300">
                      {t('admin.tickets.nextStepLabel')}
                    </p>
                    <p className="mt-1 text-sm font-semibold text-dark-100">{nextStep.title}</p>
                  ...
                  <span>
                    ...
                    {selectedTicket.user?.telegram_id && (
                      <button
                        ...
                        title={t('admin.tickets.copyTelegramId')}
                      >
                        {t('admin.tickets.telegramTag')}: {selectedTicket.user!.telegram_id}
                      </button>
                    )}{' '}
