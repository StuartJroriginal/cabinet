...
export default function Subscription() {
  ...
  const nextAction = !subscription
    ? {
        title: t('subscription.nextAction.buyTitle', 'Начните с покупки подписки'),
        description: t(
          'subscription.nextAction.buyDesc',
          'Выберите тариф и оформите подписку, чтобы получить доступ к подключению.',
        ),
        cta: t('subscription.nextAction.buyCta', 'Купить подписку'),
        onClick: () => navigate('/subscription/purchase'),
      }
    : subscription.is_limited
      ? {
          title: t('subscription.nextAction.trafficTitle', 'Закончился трафик'),
          description: t(
            'subscription.nextAction.trafficDesc',
            'Пополните баланс и докупите трафик, чтобы восстановить скорость и доступ.',
          ),
          cta: t('subscription.nextAction.trafficCta', 'Пополнить баланс'),
          onClick: () => navigate('/balance'),
        }
      : !subscription.is_active || subscription.status === 'disabled'
        ? {
            title: t('subscription.nextAction.renewTitle', 'Подписку нужно продлить'),
            description: t(
              'subscription.nextAction.renewDesc',
              'Продлите подписку, чтобы снова получить доступ к серверам.',
            ),
            cta: t('subscription.nextAction.renewCta', 'Продлить подписку'),
            onClick: () => navigate('/subscription/purchase'),
          }
        : {
            title: t('subscription.nextAction.connectTitle', 'Подключитесь к сервису'),
            description: t(
              'subscription.nextAction.connectDesc',
              'Откройте экран подключения и используйте ссылку или конфиг для вашего устройства.',
            ),
            cta: t('subscription.nextAction.connectCta', 'Перейти к подключению'),
            onClick: () =>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-accent-500/25 bg-accent-500/10 p-4 sm:p-5">
        <div className="text-xs font-semibold uppercase tracking-wide text-accent-300">
          {t('subscription.nextAction.label', 'Главное действие')}
        </div>
        <h2 className="mt-2 text-lg font-semibold text-dark-50">{nextAction.title}</h2>
        <p className="mt-1 text-sm text-dark-300">{nextAction.description}</p>
        <button
          type="button"
          onClick={nextAction.onClick}
          className="mt-4 inline-flex items-center rounded-xl bg-accent-500 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-accent-600"
        >
          {nextAction.cta}
        </button>
      </div>
          ...
          return (
            ...
            >
              ...
              {subscription.subscription_url && !subscription.hide_subscription_link && (
                <div className="mb-5">
                  <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-dark-200">
                    {t('subscription.copyLink')}
                  </div>
                  <div className="flex gap-2">
                  <code
                    className="scrollbar-hide flex-1 overflow-x-auto break-all rounded-[10px] px-3 py-2.5 font-mono text-xs leading-relaxed text-dark-100"
                    style={{
                      background: g.codeBg,
                  ...
                  <button
                    onClick={copyUrl}
                    className="flex h-auto items-center rounded-[10px] px-3.5 text-sm font-semibold transition-colors duration-300"
                    style={{
                      background: copied ? 'rgba(var(--color-accent-400), 0.18)' : g.innerBorder,
                      border: copied
                        ? '1px solid rgba(var(--color-accent-400), 0.35)'
                        : `1px solid ${g.trackBg}`,
                      color: copied ? 'rgb(var(--color-accent-400))' : 'rgb(var(--color-dark-100))',
                    }}
              ...
              {subscription.servers && subscription.servers.length > 0 && (
                <div className="mb-5">
                  <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-dark-300">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {subscription.servers.map((server) => (
                      <span
                        key={server.uuid}
                        className="inline-flex items-center gap-1.5 rounded-[8px] px-2.5 py-1.5 text-xs font-semibold text-dark-200"
                        style={{
