...
function NextStepCard({
  ...
  return (
    <div className="w-full rounded-xl border border-accent-500/25 bg-accent-500/10 p-4 text-left">
      <p className="text-xs font-semibold uppercase tracking-wide text-accent-300">{label}</p>
      <p className="mt-2 text-sm font-semibold text-dark-100">{title}</p>
      <p className="mt-1 text-xs leading-relaxed text-dark-300">{description}</p>
    </div>
  );
}

type ManualCreditBannerProps = {
  message: string | null;
  etaMinutes: number;
  supportUrl: string | null;
};

function ManualCreditNotice({ message, etaMinutes, supportUrl }: ManualCreditBannerProps) {
  const { t } = useTranslation();
  const { openLink } = usePlatform();
  const text =
    message ||
    t('balance.manualCreditMessage', {
      defaultValue:
        'После оплаты зачисление выполняется вручную. Обычно это занимает до {{minutes}} минут.',
      minutes: etaMinutes,
    });

  return (
    <div className="w-full rounded-xl border border-warning-500/30 bg-warning-500/10 p-4 text-left">
      <p className="text-sm font-semibold text-warning-200">
        {t('balance.manualCreditTitle', { defaultValue: 'Зачисление вручную' })}
      </p>
      <p className="mt-2 text-xs leading-relaxed text-warning-100/90">{text}</p>
      {supportUrl ? (
        <button
          type="button"
          onClick={() => openLink(supportUrl)}
          className="mt-3 w-full rounded-lg border border-warning-400/50 px-3 py-2 text-xs font-semibold text-warning-100 transition-colors hover:bg-warning-500/20"
        >
          {t('common.support', { defaultValue: 'Поддержка' })}
        </button>
      ) : null}
    </div>
  );
...
function PendingState({
  ...
  manualCredit: ManualCreditBannerProps | null;
  onGoBack: () => void;
}) {
  const { t } = useTranslation();
  ...
  return (
    ...
    >
      ...
      <NextStepCard
        label={t('balance.topUpResult.nextStepLabel', 'Следующий шаг')}
        title={t('balance.topUpResult.pendingNextTitle', 'Дождитесь подтверждения платежа')}
        description={t(
          'balance.topUpResult.pendingNextDesc',
          'Обычно это занимает 1-2 минуты. Если статус не меняется, вернитесь в баланс и проверьте историю платежей.',
        )}
      />
      {manualCredit ? (
        <ManualCreditNotice
          message={manualCredit.message}
          etaMinutes={manualCredit.etaMinutes}
          supportUrl={manualCredit.supportUrl}
        />
      ) : null}
...
function SuccessState({ amountKopeks }: { amountKopeks: number | null }) {
  ...
  return (
    ...
    >
      ...
      <NextStepCard
        label={t('balance.topUpResult.nextStepLabel', 'Следующий шаг')}
        title={t('balance.topUpResult.successNextTitle', 'Продолжите с подпиской')}
        description={t(
          'balance.topUpResult.successNextDesc',
          'Теперь можно вернуться в баланс и перейти к покупке или продлению подписки.',
        )}
      />
...
function FailedState({ amountKopeks }: { amountKopeks: number | null }) {
  ...
  return (
    ...
    >
      ...
      <NextStepCard
        label={t('balance.topUpResult.nextStepLabel', 'Следующий шаг')}
        title={t('balance.topUpResult.failedNextTitle', 'Попробуйте другой способ')}
        description={t(
          'balance.topUpResult.failedNextDesc',
          'Вернитесь в баланс и выберите другой платежный метод, чтобы завершить пополнение.',
        )}
      />
...
function TimeoutState({
  ...
  return (
    ...
    >
      ...
      <NextStepCard
        label={t('balance.topUpResult.nextStepLabel', 'Следующий шаг')}
        title={t('balance.topUpResult.timeoutNextTitle', 'Проверьте статус ещё раз')}
        description={t(
          'balance.topUpResult.timeoutNextDesc',
          'Если вы уже оплатили, нажмите "Повторить". Если статус не обновится, обратитесь в поддержку.',
        )}
      />
      {manualCredit ? (
        <ManualCreditNotice
...
export default function TopUpResult() {
  ...
  const isOzonSbpMethod =
    (pendingInfo?.method_id || '').toLowerCase() === 'ozon_sbp' ||
    (methodFromUrl || '').toLowerCase() === 'ozon_sbp';

  const manualCreditBanner: ManualCreditBannerProps | null =
    isOzonSbpMethod && !resolvedPaid && !resolvedFailed
      ? {
          message: pendingInfo?.manual_credit_message ?? null,
          etaMinutes: pendingInfo?.manual_credit_eta_minutes ?? 15,
          supportUrl: pendingInfo?.support_contact_url ?? null,
        }
      : null;
      ...
      >
        {resolvedPaid ? (
          ...
          <TimeoutState
            ...
            manualCredit={manualCreditBanner}
          />
        ) : (
          <PendingState
