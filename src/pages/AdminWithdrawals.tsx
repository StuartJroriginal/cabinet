...
function getWithdrawalNextStep(
  t: (key: string) => string,
  status: AdminWithdrawalItem['status'],
): { title: string; description: string } {
  if (status === 'pending') {
    return {
      title: t('admin.withdrawals.nextStep.pendingTitle'),
      description: t('admin.withdrawals.nextStep.pendingDesc'),
    };
  }
  if (status === 'approved') {
    return {
      title: t('admin.withdrawals.nextStep.approvedTitle'),
      description: t('admin.withdrawals.nextStep.approvedDesc'),
    };
  }
  if (status === 'rejected' || status === 'cancelled') {
    return {
      title: t('admin.withdrawals.nextStep.rejectedTitle'),
      description: t('admin.withdrawals.nextStep.rejectedDesc'),
    };
  }
  return {
    title: t('admin.withdrawals.nextStep.completedTitle'),
    description: t('admin.withdrawals.nextStep.completedDesc'),
  };
}
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {items.map((item: AdminWithdrawalItem) => {
            ...
            const nextStep = getWithdrawalNextStep(t, item.status);
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 rounded-lg border border-accent-500/20 bg-accent-500/10 p-3">
                      <p className="text-xs font-semibold uppercase tracking-wide text-accent-300">
                        {t('admin.withdrawals.nextStep.label')}
                      </p>
