...
function getPartnerNextStep(
  t: (key: string) => string,
  partner: AdminPartnerItem,
): { title: string; description: string } {
  if (partner.total_referrals === 0) {
    return {
      title: t('admin.partners.nextStep.partnerNoReferralsTitle'),
      description: t('admin.partners.nextStep.partnerNoReferralsDesc'),
    };
  }
  return {
    title: t('admin.partners.nextStep.partnerActiveTitle'),
    description: t('admin.partners.nextStep.partnerActiveDesc'),
  };
}

function getApplicationNextStep(
  t: (key: string) => string,
): { title: string; description: string } {
  return {
    title: t('admin.partners.nextStep.applicationTitle'),
    description: t('admin.partners.nextStep.applicationDesc'),
  };
}
          ...
          {partnersLoading ? (
            ...
            <div className="space-y-3">
              {partners.map((partner: AdminPartnerItem) => {
                const nextStep = getPartnerNextStep(t, partner);
                return (
                  <button
                    key={partner.user_id}
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-2 rounded-lg border border-accent-500/20 bg-accent-500/10 p-3">
                          <p className="text-xs font-semibold uppercase tracking-wide text-accent-300">
                            {t('admin.partners.nextStep.label')}
                          </p>
          ...
          {applicationsLoading ? (
            ...
            <div className="space-y-3">
              {applications.map((app: AdminPartnerApplicationItem) => {
                const nextStep = getApplicationNextStep(t);
                return (
                  ...
                  <div className="mb-3 rounded-lg border border-accent-500/20 bg-accent-500/10 p-3">
                    <p className="text-xs font-semibold uppercase tracking-wide text-accent-300">
                      {t('admin.partners.nextStep.label')}
                    </p>
