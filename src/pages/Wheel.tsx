...
export default function Wheel() {
  ...
  const [paymentType, setPaymentType] = useState<'balance' | 'telegram_stars' | 'subscription_days'>(
    'balance',
  );
  ...
  const balanceEnabled = config.spin_cost_stars_enabled && config.spin_cost_stars;
  const daysEnabled = config.spin_cost_days_enabled && config.spin_cost_days;
  const bothMethodsAvailable = !!(balanceEnabled && daysEnabled);
  ...
  const spinDisabled =
    ...
    (paymentType === 'balance'
      ? !balanceEnabled || !config.can_pay_balance
        ...
        <div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[1fr,280px]">
          ...
          <div>
            ...
            <div className="mt-8 space-y-4">
                  ...
                  >
                    {balanceEnabled && (
                      ...
                      >
                        ...
                        {(config.required_balance_kopeks / 100).toFixed(2)}
                      </button>
                    )}
                    {daysEnabled && (
                      <button
              ...
              {/* Stars confirmation panel (used only for Telegram Stars invoice mode) */}
              {paymentType === 'telegram_stars' && showStarsConfirm && !isSpinning && !isPayingStars ? (
                        ...
                        <div className="whitespace-nowrap text-sm text-dark-400">
                          ...
                          {item.payment_type === 'telegram_stars'
                            ...
                            : item.payment_type === 'balance'
                              ? `${(item.payment_value_kopeks / 100).toFixed(2)} ₽`
