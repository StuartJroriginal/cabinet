  ...
  const nextAction = useMemo(() => {
    if (isMultiTariff) {
      const hasActive = Boolean(
        multiSubData?.subscriptions?.some((sub) => sub.status === 'active' || sub.status === 'trial'),
      );
      if (hasActive) {
        return {
          title: t('dashboard.nextAction.manageTitle', 'Управляйте подписками'),
          description: t(
            'dashboard.nextAction.manageDesc',
            'Откройте список подписок, чтобы подключиться, продлить или изменить тариф.',
          ),
          to: '/subscriptions',
          cta: t('dashboard.nextAction.manageCta', 'Открыть подписки'),
        };
      }
      return {
        title: t('dashboard.nextAction.buyTitle', 'Начните с подписки'),
        description: t(
          'dashboard.nextAction.buyDesc',
          'Оформите первую подписку, чтобы получить доступ к подключению и серверным локациям.',
        ),
        to: '/subscription/purchase',
        cta: t('dashboard.nextAction.buyCta', 'Купить подписку'),
      };
    }
    ...
    if (subscription.is_expired || subscription.status === 'disabled' || subscription.is_limited) {
      return {
        title: t('dashboard.nextAction.renewTitle', 'Подписку нужно продлить'),
        description: t(
          'dashboard.nextAction.renewDesc',
          'Продлите подписку, чтобы восстановить стабильный доступ ко всем функциям.',
        ),
        to: '/subscription',
        cta: t('dashboard.nextAction.renewCta', 'Продлить подписку'),
      };
    }

    return {
      title: t('dashboard.nextAction.connectTitle', 'Подключитесь к сервису'),
      description: t(
        'dashboard.nextAction.connectDesc',
        'Скопируйте или откройте ссылку подключения в разделе подписки.',
      ),
      to: '/connection',
      cta: t('dashboard.nextAction.connectCta', 'Перейти к подключению'),
    };
  }, [isMultiTariff, multiSubData?.subscriptions, subscription, t]);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-accent-500/25 bg-accent-500/10 p-4 sm:p-5">
        <div className="text-xs font-semibold uppercase tracking-wide text-accent-300">
          {t('dashboard.nextAction.label', 'Следующий шаг')}
        </div>
        ...
        <Link
          to={nextAction.to}
          className="mt-4 inline-flex items-center gap-2 rounded-xl bg-accent-500 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-accent-600"
        >
          {nextAction.cta}
          <ChevronRightIcon />
        </Link>
