...
function getUserNextStep(
  t: (key: string) => string,
  user: UserListItem,
): { title: string; description: string } {
  if (user.status === 'blocked') {
    return {
      title: t('admin.users.nextStep.blockedTitle'),
      description: t('admin.users.nextStep.blockedDesc'),
    };
  }

  if (!user.has_subscription || user.subscription_status === 'expired') {
    return {
      title: t('admin.users.nextStep.noSubscriptionTitle'),
      description: t('admin.users.nextStep.noSubscriptionDesc'),
    };
  }

  if (user.subscription_status === 'limited') {
    return {
      title: t('admin.users.nextStep.limitedTitle'),
      description: t('admin.users.nextStep.limitedDesc'),
    };
  }

  return {
    title: t('admin.users.nextStep.activeTitle'),
    description: t('admin.users.nextStep.activeDesc'),
  };
}
...
function UserRow({ user, onClick, formatAmount }: UserRowProps) {
  ...
  const nextStep = getUserNextStep(t, user);
  return (
    <div
      onClick={onClick}
