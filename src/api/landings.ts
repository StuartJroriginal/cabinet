...
export const adminLandingsApi = {
  ...
  /** For landing editor; requires landings:read only (not payment_methods:read). */
  getPaymentMethodOptions: async (): Promise<PaymentMethodConfig[]> => {
    const response = await apiClient.get<PaymentMethodConfig[]>(
      '/cabinet/admin/landings/payment-method-options',
    );
    return response.data;
  },
};
